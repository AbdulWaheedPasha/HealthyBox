		...
		if ($_SERVER['HTTP_AUTHORIZATION'] != "") {
			if ($_SERVER['HTTP_AUTHORIZATION']) {
				try {
                    ...
                    if(!empty($data->access_token)  && !empty($data->user_email)){
                        ...
                        $user->social_media_id       = $data->social_media_id;
                        if($user->getForMail() == 0){
                            if($user->createUserToken()){
                                //echo "hello";
                                http_response_code(200);
                                $result         = $user->getUserByEmailToken();
                                while($user_row = mysqli_fetch_assoc($result)) {
                            
                                        $user_info = array(
                                            "social_media_id"      =>  $user_row['social_media_id'],
                                            "access_token"          =>  $user_row['adminstrator_access_token'],
                                            "user_image_path"       =>  $user_row['administration_profile'],
                                                "user_image_path"       =>  $user_row['administration_profile'],
                                                "user_id"              =>  $user_row['administration_id'],
                                                "user_name"            =>  $user_row['administration_name'],
                                                "user_email"           =>  $user_row['administration_username'],
                                                "user_password"        =>  $user_row['adminstrator_access_token'],
                            ...
                            http_response_code(200);
                            echo json_encode(array("message" => "User was found","status" => "1","user"=> $user_info));
                        }else{
                           
                                http_response_code(401);
                                echo json_encode(array("message" => "Email and token wasn't Found","status" => "0"));
