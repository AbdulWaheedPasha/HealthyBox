...
if (isset($_SESSION['user_name']) || isset($_SESSION['password'])) {
    if(!empty($_POST['RealUserName']) && !empty($_POST['Telephone']) 
    && !empty($_POST['username']) && !empty($_POST['password'])  
    && count($_POST['area_id']) > 0 && !empty($_POST['password']) 
    && !empty($_POST['username']) ){
        ...
        if($user_address_controller->search_username_password_register() > 0){
            ...
            $query  = "INSERT INTO `administration_tbl` (`administration_name`,`administration_username`, `administration_password`,`administration_type_id`, `administration_active`, `branch_id`,`administration_telephone_number`,`administration_date_registeration`, `administration_addres1`) VALUES ('$RealUserName','$UserName', '$Password', '4','1','$branch_id','$Telephone','$registered_time','')";
            //echo $query;
            ...
            if ($rs) {
               ...
               // echo count($area_id);
                for($i=0;$i<count($_POST['area_id']);$i++){
                    ...
                    $query_pro  = "INSERT INTO `driver_capital_tbl`(`driver_id`, `capital_id`) VALUES ('$last_id','$area_id_1') ";
                    $insert_pro = mysqli_query($con, $query_pro);
